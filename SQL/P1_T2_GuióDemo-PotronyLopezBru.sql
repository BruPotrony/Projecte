...
CREATE TABLE USUARI (
	login VARCHAR2 (30) CONSTRAINT PK_USUARI PRIMARY KEY 
    	...
    	nom VARCHAR2 (40) CONSTRAINT NN_NOM_USUARI NOT NULL 
			CONSTRAINT CK_NOM_USUARI CHECK ((LENGTH (nom) BETWEEN 2 AND 40) AND REGEXP_LIKE (nom, '^[A-Za-zàáèéíïòóúüçñÀÁÈÉÍÏÒÓÚÜÇÑ]+$')),
...
CREATE TABLE CATEGORIA (
	...
	nom VARCHAR2 (40) CONSTRAINT UN_NOM_CATEGORIA UNIQUE 
			...
			CONSTRAINT CK_NOM_CATEGORIA CHECK ((LENGTH(nom) > 1 AND LENGTH(nom) < 40) AND (REGEXP_LIKE(nom, '^[A-Za-z àáèéíïòóúüçñÀÁÈÉÍÏÒÓÚÜÇÑ]+$'))),
...
CREATE TABLE JUGADOR (
	...
	nom VARCHAR2(40) CONSTRAINT NN_NOM_JUGADOR NOT NULL
			CONSTRAINT CK_NOM_JUGADOR CHECK (LENGTH(nom) > 1 AND LENGTH(nom) < 40 AND REGEXP_LIKE(nom, '^[A-Za-zàáèéíïòóúüçñÀÁÈÉÍÏÒÓÚÜÇÑ]+$')),
	cognom VARCHAR2(40) CONSTRAINT NN_COGNOM_JUGADOR NOT NULL 
        		CONSTRAINT CK_COGNOM_JUGADOR CHECK (LENGTH(cognom) > 1 AND LENGTH(cognom) < 40 AND REGEXP_LIKE(cognom, '^[A-Za-zàáèéíïòóúüçñÀÁÈÉÍÏÒÓÚÜÇÑ]+$')),
	...
	codiPostal VARCHAR2(5) CONSTRAINT NN_CODI_POSTAL_JUGADOR NOT NULL
        CONSTRAINT CK_CODI_POSTAL_JUGADOR CHECK (LENGTH(codiPostal) = 5),
	poblacio VARCHAR2(100) CONSTRAINT NN_POBLACIO_JUGADOR NOT NULL
    	CONSTRAINT CK_POBLACIO_JUGADOR CHECK (LENGTH(poblacio) > 1 AND LENGTH(poblacio) < 51 AND REGEXP_LIKE(poblacio, '^[A-Za-zàáèéíïòóúüçñÀÁÈÉÍÏÒÓÚÜÇÑ ]+$')),
